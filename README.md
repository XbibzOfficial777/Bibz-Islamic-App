...
Prayer notification scheduling uses the versioned Android channel `quranx_prayer_adhan_v4`, the explicit Android resource URI `android.resource://com.all.bibz/raw/quranx_adhan`, and Android alarm audio usage. The Jadwal Sholat screen includes `Uji audio adzan` so the native path can be tested immediately. Using an explicit URI avoids the raw-name validation path that produced `invalid_sound` on v1.6.2. If Android still rejects the bundled sound, QuranX records the original failure as a warning and schedules the notification through a separate system-sound channel rather than silently dropping the reminder. Android notification-channel sound settings are persistent, so an existing silent channel may require updating the app, clearing the QuranX notification channel, or reinstalling before its sound configuration changes.
