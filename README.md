...
The supplied `adan_madinah_32_16.mp3` is bundled as `assets/audio/adhan_madinah.mp3` for Flutter access and `android/app/src/main/res/raw/quranx_adhan.mp3` for native Android MediaPlayer playback. The visual notification channel is kept silent to avoid duplicate audio; the native alarm receiver starts a foreground MediaPlayer service at the scheduled time.
...
Prayer reminders use the Android channel `quranx_prayer_adhan_v5` for the visual notification and a native Android alarm/foreground `MediaPlayer` path for the bundled `quranx_adhan.mp3`. This avoids relying on the notification channel to play a long adzan file, which can result in vibration without sound when the channel or notification volume is muted. The Jadwal Sholat screen includes `Uji audio adzan` for an immediate native playback test. Native playback failures are persisted and surfaced in Log Kesalahan when QuranX next opens. Android may still block playback through silent mode, zero alarm/media volume, Do Not Disturb, battery restrictions, or OEM policy; the app cannot override those device controls.
