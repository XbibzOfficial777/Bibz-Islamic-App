...
The Jadwal Sholat screen supports both automatic GPS refresh and debounced manual city search, displays the verified daily schedule, and stores the selected city ID and name in local preferences. The saved location is restored and shown when the screen is opened again; the `Perbarui Lokasi` button remains available for a fresh GPS lookup. Notifications for Subuh, Dzuhur, Ashar, Maghrib, and Isya use the device's native IANA timezone and are scheduled inexactly to avoid claiming exact-alarm privileges that may not be appropriate for every distribution channel. Android/OEM restrictions may still affect delivery.
...
The prayer settings flow supports two explicit sources: `Perbarui Lokasi` uses GPS and automatic reverse-geocoding, while the city search field queries the existing QuranX city API manually. Both paths validate the returned city/date/source fields, fetch the schedule from the same API, and persist the selected city for the next screen opening.

The supplied `adan_madinah_32_16.mp3` is bundled as `assets/audio/adhan_madinah.mp3` for Flutter access and `android/app/src/main/res/raw/quranx_adhan.mp3` for Android notification sound playback. The notification channel uses a versioned channel ID because Android notification-channel sound settings persist after channel creation.
...
The GPS prayer resolver now tries the original reverse-geocoded locality and normalized variants, removing administrative prefixes such as `Kecamatan`, `Kabupaten`, `Kab.`, and `Kota` before querying the QuranX city catalog. This allows a location such as `Kecamatan Tulungagung` to resolve to the verified API city `KAB. TULUNGAGUNG` (city ID `1629`) without reintroducing manual city typing or selecting an unrelated result arbitrarily.

When the user taps the GPS action, QuranX requests location permission automatically if it has not been granted. If permission was permanently denied, the app offers to open QuranX application settings. If the device location service is disabled, the app explains the requirement and offers to open the system location settings; Android does not allow an application to silently enable the device GPS switch itself.

Prayer notification scheduling uses the versioned Android channel `quranx_prayer_adhan_v3`, the raw resource `quranx_adhan`, and Android alarm audio usage. The Jadwal Sholat screen includes `Uji audio adzan` so the native path can be tested immediately. If a device or build rejects the bundled raw sound, QuranX records the original failure as a warning and schedules the notification through a separate system-sound channel rather than silently dropping the reminder. The Jadwal Sholat screen shows the next prayer and a live countdown in hours, minutes, and seconds; once the scheduled time arrives, Android plays the configured notification sound subject to channel, volume, Do Not Disturb, and OEM settings. Android notification-channel sound settings are persistent, so an existing silent channel may require updating the app, clearing the QuranX notification channel, or reinstalling before its sound configuration changes.
